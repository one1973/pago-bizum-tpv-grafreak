 ...
 * @subpackage Pago_Bizum_Grafreak/public
 */
...
/**
 * Class Pago_Bizum_Grafreak_Public
 */
class Pago_Bizum_Grafreak_Public {
	...
	public function enqueue_styles() {
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/pago-bizum-grafreak-public.css', array(), $this->version, 'all' );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/pago-bizum-grafreak-public.js', array( 'jquery' ), $this->version, false );
...
// AÑADIDO PARA PAGAR POR BIZUM
	...
	public function pago_tpv_shortcode( $atts, $content = null ) {
		...
		if ( 'enabled' === $enable ) {
			ob_start();
			include 'partials/pago-bizum-grafreak-public-display.php';
	...
	public function pago_tpv_ok_shortcode( $atts, $content = null ) {
		...
		if ( isset( $_REQUEST['Ds_Signature'] ) && 'enabled' === $enable ) {
			...
			if ( $firma === $signaturerecibida && intval( $decodec['Ds_Response'] ) < 100 ) {
				...
				<h2><?php esc_html_e( 'Payment complete', 'pago-bizum-grafreak' ); ?></h2>
					<table class="table-result-tpv">
						<tr>
							<td><?php esc_html_e( 'Date:', 'pago-bizum-grafreak' ); ?></td>
						...
						<tr>
							<td><?php esc_html_e( 'Quantity:', 'pago-bizum-grafreak' ); ?></td>
