 ...
 * @subpackage Pago_Bizum_Grafreak/public/partials
 */
...
if ( ! isset( $_REQUEST['Ds_SignatureVersion'] ) ) { // FORMULARIO PRE ENVIO. ?>
	...
	<table class="table-form-tpv">
		<tr class="tpv-plugin-codigo-pedido">
			<td><?php esc_html_e( 'Order code', 'pago-bizum-grafreak' ); ?></td>
			<td><span class="error"><?php esc_html_e( 'Must be between 4 and 8 characters', 'pago-bizum-grafreak' ); ?></span><input type="text" value="<?php echo esc_html( $inputnp ); ?>" name="orderNumber" id="orderNumber" <?php echo esc_html( $readonlynp ); ?> /></td>
		</tr>
        <tr class="tpv-plugin-desc-pedido">
            <td><?php esc_html_e( 'Order Description', 'pago-bizum-grafreak' ); ?></td>
            <td><span class="error"><?php esc_html_e( 'Must have less than 125 characters', 'pago-bizum-grafreak' ); ?></span><input type="text" value="<?php echo esc_html( $inputdesc ); ?>" name="orderNumber" id="orderDesc" <?php echo esc_html( $readonlydesc ); ?> /></td>
        </tr>
		<tr class="tpv-plugin-cantidad-pagar">
			<td><?php esc_html_e( 'Quantity to pay', 'pago-bizum-grafreak' ); ?></td>
			<td><span class="error"><?php esc_html_e( 'Must specify a quantity', 'pago-bizum-grafreak' ); ?></span><input type="number" step="0.01" value="<?php echo esc_html( $inputc ); ?>" name="amountTPV" id="amountTPV" <?php echo esc_html( $readonlyc ); ?> /><span>€<span></td>
		</tr>
	</table>
	<div class="section-right-form-tpv">
		...
		<?php } ?>
		<a href="#" id="form_tpv_submit"><?php esc_html_e( 'Pay', 'pago-bizum-grafreak' ); ?></a>
	</div>
</div>
<?php 
	} else { 
		$BizumAPI_Grafreak = new BizumAPI_Grafreak();
		$params_bizum = $BizumAPI_Grafreak->decode_merchant_parameters( $_REQUEST['Ds_MerchantParameters'] );
		$params_bizum = json_decode( $params_bizum );
		echo '<p class="bizum_response_description">' . urldecode( esc_html( sanitize_text_field( $params_bizum->Ds_Response_Description) ) ) . '</p>';
	} 
?>